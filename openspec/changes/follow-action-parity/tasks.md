...
- [x] 1.1 Add `FOLLOWEES: "15"` to `enums.BanSource` in
      `frontend/app/assets/js/enums.js`.
- [x] 1.2 Add a new Django migration in `backend/django_EksiEngel/api/migrations/`
...
- [x] 1.3 Add the mirrored migration in
...
- [x] 2.1 In `background.js`'s `processHandler`, `BanSource.SINGLE` branch
...
- [x] 3.1 Add "yazarı takip et" button: dispatches `BanSource.SINGLE`,
...
- [x] 3.2 Add "favlayanları takip et" button: dispatches `BanSource.FAV`,
      `BanMode.BAN`, `TargetType.FOLLOW`. Insert after "favlayanları engelle".
- [x] 3.3 Add "takipçilerini takip et" button: dispatches `BanSource.FOLLOW`,
      `BanMode.BAN`, `TargetType.FOLLOW`. Insert after "takipçilerini
      engelle/sessize al".
- [x] 3.4 Add "takip ettiklerini takip et" button: dispatches
...
- [x] 4.1 Add a "takip et" button next to the profile's existing block/mute
      button: dispatches `BanSource.SINGLE`, `BanMode.BAN`, `TargetType.FOLLOW`
      for the profile's author.
- [x] 4.2 Add "takipçilerini takip et" next to the existing "takipçilerini
      engelle/sessize al" button: dispatches `BanSource.FOLLOW`, `BanMode.BAN`,
      `TargetType.FOLLOW`.
- [x] 4.3 Add "takip ettiklerini takip et": dispatches `BanSource.FOLLOWEES`,
      `BanMode.BAN`.

## 5. processHandler: FAV and FOLLOW (followers) branches gain follow mode

- [x] 5.1 In the `BanSource.FAV` loop (~background.js:790), branch on
...
- [x] 5.2 Apply the same targetType branch to the `BanSource.FOLLOW`
...
- [x] 6.1 Add an `else if (banSource === enums.BanSource.FOLLOWEES)` branch in
...
- [x] 7.3 `cd frontend/app && npm run check && npm run package` — confirm the
