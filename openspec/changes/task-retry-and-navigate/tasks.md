...
- [x] 1.1 Add `OPERATION_RETRY` to `enums.ClickType` in `frontend/app/assets/js/enums.js`.
- [x] 1.2 Extend the `metadata` object built alongside single-target dispatches
...
- [x] 1.3 Thread the same fields through to whatever record backs the completed
...
- [x] 1.4 Confirm via `resumableOperation.js` checkpoint persistence that these
...
- [x] 2.1 In `notification.js`, add a "Tekrarla" button/handler to
      `updatePlannedProcessesTable` rows, rendered only when the row's record has
      the target-identity fields from Task 1.
- [x] 2.2 Add the same button/handler to `insertCompletedProcessesTable` rows
      (both FAILED and successful), same visibility rule.
- [x] 2.3 Implement the click handler: build the same message shape
...
- [x] 2.4 Send `commHandler.sendAnalyticsData({ click_type:
...
- [x] 3.1 Determine the existing title→URL construction used elsewhere for
...
- [x] 3.2 Add a "Git" button/handler to both task tables, rendered when the
...
- [x] 3.3 Implement the click handler using `chrome.tabs.create({ url })`,
...
- [x] 4.1 Add "Tekrarla"/"Git" button styles to
...
- [x] 5.3 `cd frontend/app && npm run check && npm run package` — confirm the
